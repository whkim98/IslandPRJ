...
@Controller
public class Cat22Controller {
	
	private MngBrdService mngbrdService;
	
	public Cat22Controller(MngBrdService mngbrdService) {
		this.mngbrdService = mngbrdService;
	}
	
	
	@RequestMapping("/cat22/list")
	public String list(Model model, Integer page, String type, String word) {
		...
		return ViewPath.CAT22 + "list.jsp";
	}

	@RequestMapping("/cat22/writeform")
	public String insertForm() {
		return ViewPath.CAT22 + "write.jsp";
	}
	
	@Autowired
	private ServletContext application;
	
	@RequestMapping("/cat22/write")
	public String insert(Model model, HttpSession session, MngBrdVO vo) {
				
		Integer no = (Integer)session.getAttribute("login");
		vo.setManager_no(no);
		
		int num = mngbrdService.MngBrdGetNum22();
		vo.setNum(num);
		...
		model.addAttribute("url", "/kisland/cat22/list");
		
		model.addAttribute("vo", vo);

		return ViewPath.CAT22 + "result.jsp";
	}
	
	
	@RequestMapping("/cat22/content")
	public String content(Model model, int no) {
		...
		return ViewPath.CAT22 + "content.jsp";
	}
	
	@RequestMapping("/cat22/updateform")
	public String updateForm(Model model, int no) {
		...
		return ViewPath.CAT22 + "update.jsp";
	}
	
	@RequestMapping("/cat22/update")
	public String update(Model model, MngBrdVO vo) {
		...
		model.addAttribute("url", "/kisland/cat22/content?no=" + vo.getNo());
				
		return ViewPath.CAT22 + "result.jsp";
	}
	
	@RequestMapping("/cat22/delete")
