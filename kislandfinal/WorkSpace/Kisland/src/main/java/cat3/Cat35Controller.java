...
@Controller
public class Cat35Controller {
	
	private MngBrdService mngbrdService;
	
	public Cat35Controller(MngBrdService mngbrdService) {
		this.mngbrdService = mngbrdService;
	}
	
	
	@RequestMapping("/cat35/list")
	public String list(Model model, Integer page, String type, String word) {
		...
		return ViewPath.CAT35 + "list.jsp";
	}

	@RequestMapping("/cat35/writeform")
	public String insertForm() {
		return ViewPath.CAT35 + "write.jsp";
	}
	
	@Autowired
	private ServletContext application;
	
	@RequestMapping("/cat35/write")
	public String insert(Model model, HttpSession session, MngBrdVO vo) {
				
		Integer manager_no = (Integer)session.getAttribute("manager_no");
		vo.setManager_no(manager_no);
		
		int num = mngbrdService.MngBrdGetNum35();
		vo.setNum(num);
		...
		model.addAttribute("url", "/ki/cat35/list");
		
		model.addAttribute("vo", vo);

		return ViewPath.CAT35 + "result.jsp";
	}
	
	
	@RequestMapping("/cat35/content")
	public String content(Model model, int no) {
		...
		return ViewPath.CAT35 + "content.jsp";
	}
	
	@RequestMapping("/cat35/updateform")
	public String updateForm(Model model, int no) {
		...
		return ViewPath.CAT35 + "update.jsp";
	}
	
	@RequestMapping("/cat35/update")
	public String update(Model model, MngBrdVO vo) {
		...
		model.addAttribute("url", "/ki/cat35/content?no=" + vo.getNo());
				
		return ViewPath.CAT35 + "result.jsp";
	}
	
	@RequestMapping("/cat35/delete")
