...
<div align="center">
			...
			<c:otherwise>
				<c:forEach var="dto" items="${list }">
					<tr>
						...
						<td class="center"><a href="${pageContext.request.contextPath }/cat32/content?no=${dto.no}">${dto.mngbrd_title}</a></td>
	...
	<div align="center">
		<c:if test="${paging.prev}">
			<a href="${pageContext.request.contextPath}/cat32/list?page=${paging.begin-1}&type=${param.type}&word=${param.word}">[이전]</a>
		</c:if>
		...
		<c:forEach var="i" begin="${paging.begin}" end="${paging.end}" step="1">
			<c:choose>
				...
				<c:otherwise>
					<a href="${pageContext.request.contextPath}/cat32/list?page=${i}&type=${param.type}&word=${param.word}">[${i}]</a>
				</c:otherwise>
			</c:choose>
		</c:forEach>
		<c:if test="${paging.next}">
			<a href="${pageContext.request.contextPath}/cat32/list?page=${paging.end+1}&type=${param.type}&word=${param.word}">[다음]</a>
		</c:if>
	</div>
	...
	<c:if test="${!empty mngcheck }">
		<div class="btnBox">
			<button class="btn" onclick="document.location.href='${pageContext.request.contextPath }/cat32/writeform'">관리자 글쓰기</button>
		</div>
	</c:if>
	...
	<div class="col-md-4 mb-3">
	<form action="${pageContext.request.contextPath }/cat32/list" method="get" onsubmit="return check(this)" >
